...
func (t *SimpleChaincode) createItem(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	it.Id 			= args[0]
	it.Name			= args[1]
	it.CurrentOwner	= args[2]
	it.Manufacturer	= args[2]
	it.Barcode 		= args[3]
...
func (t *SimpleChaincode) confirmOwnership(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	if len(args) != 4 {
		fmt.Println("Incorrect number of arguments. Expecting 4 (ItemId, user, date, location)")
...
func (t *SimpleChaincode) transferOwnership(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	if args[1] != itm.CurrentOwner { return nil, errors.New("You are not allowed to transfer an item") }
...
func (t *SimpleChaincode) changeStatus(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	if args[1] != itm.CurrentOwner { return nil, errors.New("You are not allowed to mark this item with new status") }
